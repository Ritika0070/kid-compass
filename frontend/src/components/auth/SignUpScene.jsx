...
const groupVariants = {
  ...
  animate: (direction) => ({
    y: direction === "login-signup" ? [-1000, 0, -15, 0] : 0,
    transition:
  ...
  exit: (direction) => {
    if (direction === "signup-verify") {
      return { y: "60vh", transition: { duration: 0.38, ease: EASE_IN_ACCEL } };
    }
    if (direction === "signup-login") {
      return { y: "-60vh", transition: { duration: 0.35, ease: EASE_IN_ACCEL } };
    }
    return { y: 0 };
  },
};

export default function SignUpScene({ onLogin, onSubmit, direction }) {
    ...
    <>
      ...
      <div className="flex md:hidden relative z-10 flex-col items-center justify-center w-full h-full px-6 py-8 gap-4">
        <div className="w-full max-w-sm">
          <form
            onSubmit={handleSubmit}
            className="w-full bg-[#FAF9F0] shadow-lg box-border relative"
            style={{
